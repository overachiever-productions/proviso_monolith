	...
	Group-Definitions -GroupKey "AdminDb.*" {
		Definition "IOAlertsEnabled" -ExpectValueForChildKey "Alerts.IOAlertsEnabled" {
			Test {
				...
				$PVContext.AddFacetState("$instanceName.IoAlertsEnabled", $true);
				return $true;
			}
			Configure {
				$instanceName = $PVContext.CurrentKeyValue;
				...
				if ($PVContext.GetFacetState("$instanceName.IoAlertsEnabled") -and (-not ($ioAlertsEnabled))) {
				...
				if ($PVContext.GetFacetState("$instanceName.SeverityAlertsEnabled") -and (-not ($severityAlertsEnabled))) {
				...
				if ($PVContext.GetFacetState("$instanceName.IOAlertsFiltered") -and (-not ($ioAlertsFiltered))) {
				...
				if ($PVContext.GetFacetState("$instanceName.SeverityAlertsFiltered") -and (-not ($severityAlertsFiltered))) {
		...
		Definition "SeverityAlertsEnabled" -ExpectValueForChildKey "Alerts.SeverityAlertsEnabled" -ConfiguredBy "IOAlertsEnabled" {
			Test {
				...
				$PVContext.AddFacetState("$instanceName.SeverityAlertsEnabled", $true);
				return $true;
			}
		}
		...
		Definition "IOAlertsFiltered" -ExpectValueForChildKey "Alerts.IOAlertsFiltered" -ConfiguredBy "IOAlertsEnabled" {
			Test {
				...
				$PVContext.AddFacetState("$instanceName.IOAlertsFiltered", $true);
				if ($count -eq 4){
					return $true;
				}
		...
		Definition "SeverityAlertsFiltered" -ExpectValueForChildKey "Alerts.SeverityAlertsFiltered" -ConfiguredBy "IOAlertsEnabled" {
			Test {
				...
				$PVContext.AddFacetState("$instanceName.SeverityAlertsFiltered", $true);
				if ($count -eq 9) {
					return $true;
				}
