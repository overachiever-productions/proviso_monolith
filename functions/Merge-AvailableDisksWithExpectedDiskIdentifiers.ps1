...
function Merge-AvailableDisksWithExpectedDiskIdentifiers {
	[CmdletBinding()]
	param (
	...
	begin {
		if ($CurrentlyMountedVolumeLetters.Count -lt 1) {
	...
	process {
		...
		foreach ($diskName in $keys) {
			...
			if (!($driveLetter -in $CurrentlyMountedVolumeLetters)) {
