	...
	Group-Definitions -GroupKey "AdminDb.*" {
		Definition "Deployed" -ExpectValueForChildKey "Deploy" {
			...
			Configure {
				...
				if ($expectedSetting) {
					...
					$PVContext.AddFacetState("$instanceName.AdminDb.JustInstalled", $true);
		...
		Definition "AdminDbVersion" {
			...
			Configure {
				...
				if (-not ($PVContext.GetFacetState("$instanceName.AdminDb.JustInstalled"))) {
